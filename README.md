...
| ORM | Drizzle ORM |
| LLM | 任意 OpenAI 兼容 API（默认 Kimi K2.5） |
| Embedding | 任意 OpenAI 兼容 Embedding API（默认 Qwen3-Embedding-8B） |
...
项目支持任意 OpenAI 兼容 API，只需修改 `.env` 中的环境变量即可切换模型：

| 服务商 | `LLM_BASE_URL` | `LLM_MODEL_NAME` 示例 |
|--------|----------------|----------------------|
| ModelScope（魔塔） | `https://api-inference.modelscope.cn/v1` | `moonshotai/Kimi-K2.5` |
| OpenAI | `https://api.openai.com/v1` | `gpt-4o` |
| DeepSeek | `https://api.deepseek.com/v1` | `deepseek-chat` |
| 智谱 AI | `https://open.bigmodel.cn/api/paas/v4` | `glm-4-plus` |
| 硅基流动 | `https://api.siliconflow.cn/v1` | `Qwen/Qwen3-32B` |

> **注意**：切换 Embedding 模型后，向量维度可能不同，需要重新生成笔记的向量嵌入。
